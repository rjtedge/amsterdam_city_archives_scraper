...
directory = inventory_number.to_s + archive_ref.to_s.split(".").join("-").to_s + "/"
FileUtils.mkdir_p directory
errored = []

n = 1
imageurls.each do |save|
    name = inventory_number.to_s + archive_ref.to_s.split(".").join("-").to_s + "/" + "CAA" + inventory_number.to_s + archive_abriviation + archive_ref.to_s.split(".").join("-").to_s + "p" + n.to_s + ".jpg"
    if File.file?(name) == false
        open(name, 'wb' ) do |file|
            p "Getting image " + n.to_s + " of " + imageurls.count.to_s
            begin
...
while (errored.empty? == false) && (v <=2)
    ...
    errored.each do |save|
        name = inventory_number.to_s + archive_ref.to_s.split(".").join("-").to_s + "/" + "CAA" + inventory_number.to_s + archive_abriviation + archive_ref.to_s.split(".").join("-").to_s + "p" + save[1].to_s + ".jpg"
        p name
        if File.file?(name) == false
            open(name, 'wb' ) do |file|
                begin
